...
class Handler(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if self.path == "/api/state":
            ...
            self.wfile.write(json.dumps(s, default=str).encode())
        elif self.path == "/" or self.path == "/index.html" or self.path.startswith("/assets/"):
            # React SPA 優先；/legacy/ 走舊單檔 dashboard
            if self.path.startswith("/assets/"):
                fn = os.path.basename(self.path)
                fp = os.path.join("/root/rh_live/paper_trade/spa/assets", fn)
                ...
                return
            SPA_FILE = "/root/rh_live/paper_trade/spa/index.html"
            paper = load_json(PAPER_STATE)
            live = _live_state_cached()
            if self.path.startswith("/legacy") or not os.path.exists(SPA_FILE):
                html = open(DASHBOARD, "rb").read().decode()
                html = inject_ssr(html, paper, live)
            else:
                html = open(SPA_FILE, "rb").read().decode()
                ssr = json.dumps({"paper": paper, "live": live}, default=str)
                html = html.replace("<head>", "<head><script>window.__SSR=" + ssr + ";</script>", 1)
