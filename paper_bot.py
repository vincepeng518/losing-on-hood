...
PER_TRADE = 10.0  # 2026-09-04 對齊實盤 (grok_bot 預設 5)
...
def paper_tick():
    ...
    th["min_sniper"] = 1
    # 2026-09-04: alloc 對齊實盤 $10 (live PER_TRADE=10; 舊 5 = paper 小注試錯, 與實盤口徑不一致)
    # 覆寫 paper_bot 內 min(PER_TRADE, cash*0.4, 5.0) 的 5.0 上限 → 10
    # paper 沒有 min_narrative 限制（RH chain 幣名多不規則）
    ep = eth_price()
    # 2026-09-04: paper alloc 對齊實盤 $10 (grok_bot.PER_TRADE=5, live=10)
    ...
    for addr in list(s["positions"]):
        ...
        def _crash_vel(addr):
            """出場當下 4 根 1m K 跌幅 → crash_vel 比例(0.07~0.30 映射 0~6pt). 值域 >0 才生效."""
            try:
                ks = klines_res(addr, "1m", 6)
                if not ks or len(ks) < 5:
                    return 0.0
                closes = [float(k["close"]) for k in ks[-5:-1]]  # 最後 4 根已收盤 K
                if min(closes) <= 0:
                    return 0.0
                drop = (closes[0] - min(closes)) / closes[0]
                return drop if drop > 0.05 else 0.0
            except Exception:
                return 0.0
    ...
    for c in candidates:
        ...
        alloc_est = min(PER_TRADE, s["cash_usd"] * 0.4, 10.0)
        ...
        s["positions"][addr] = {
            ...
            "entry_eth": alloc_usd / ep, "gas_usd": total_gas,
            "snap": c.get("snap"),
