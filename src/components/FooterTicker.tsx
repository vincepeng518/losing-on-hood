...
export const FooterTicker: React.FC = () => {
  ...
  return (
    <footer className="fixed bottom-0 left-0 right-0 z-40 h-10 border-t border-white/10 bg-neutral-950/90 backdrop-blur-xl select-none">
      <div className="mx-auto flex h-full max-w-7xl items-center justify-between px-4 font-mono text-xs text-neutral-400 overflow-hidden">
        ...
        <div className="flex items-center gap-3 shrink-0">
          ...
          <div className="hidden sm:flex items-center gap-1 rounded-md border border-rose-500/40 bg-rose-500/10 px-2 py-0.5 text-[10px] font-bold text-rose-400">
        ...
        <div className="hidden md:flex items-center gap-6 overflow-hidden px-4">
          {tickers.map((t, idx) => (
            <div key={idx} className="flex items-center gap-1.5 shrink-0 text-xs">
              ...
              <span className={t.isPositive ? 'text-emerald-400' : 'text-rose-400'}>
                {t.change}
              </span>
            </div>
          ))}
